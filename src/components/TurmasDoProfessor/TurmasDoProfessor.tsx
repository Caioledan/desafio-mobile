...
type ProfessorNavigationProp = StackNavigationProp<ProfessorStackParamList, "TurmasProfessor">

export function TurmasDoProfessor() {
    const navigation = useNavigation<ProfessorNavigationProp>()
    const {turmas, isLoading} = useProfessorTurmas();
  
    const handleTurmaButton = (turmaId: number) => {
      // navigation.navigate("TurmasDetalhes", {turmaId})
    }
  ...
  return (
    <View style={styles.container}>
      ...
      {turmas.map(turma => (
        <TurmasButton key={turma.id} turmaSerie={turma.serie} turmaNome={turma.nome} onPress={() => handleTurmaButton}/>
      ))}
    </View>
  );
}