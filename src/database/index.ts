...
export function getNotaByAlunoDisciplina(
  alunoId: number,
  disciplinaId: number
) {
  return mockNotas.find(
    (nota) => nota.alunoId === alunoId && nota.disciplinaId === disciplinaId
  );
}

export function getTurmasByProfessorId(professorId: number) {
  const professorDisciplinaIds = mockDisciplinas
    .filter((disciplina) => disciplina.professorId === professorId)
  ...
  return mockTurmas.filter((turma) =>
    turma.disciplinasIds.some((disciplinaId) =>
      professorDisciplinaIds.includes(disciplinaId)
    )
  );
}
...
export function salvarNota(
  ...
  const notaExistente = mockNotas.find(
    (nota) => nota.alunoId === alunoId && nota.disciplinaId === disciplinaId
  );
